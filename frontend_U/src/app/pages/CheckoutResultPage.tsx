...
export default function CheckoutResultPage() {
  ...
  const verify = useCallback(async () => {
    ...
    try {
      ...
      setResult(data)
      if (data.payment_status === 'APPROVED') { clearCart(); sessionStorage.removeItem('pendingWompiOrder') }
  ...
  return <div className="mx-auto max-w-3xl px-4 py-14"><div className="rounded-2xl border bg-white p-8 shadow-sm">
    ...
    <dl className="mt-8 grid gap-4 rounded-xl bg-gray-50 p-6 sm:grid-cols-2">
      <div><dt className="text-sm text-gray-500">Orden</dt><dd className="font-semibold">{result.order_number}</dd></div><div><dt className="text-sm text-gray-500">Referencia</dt><dd className="break-all font-mono text-sm">{result.reference}</dd></div>
    ...
    <div className="mt-8 flex justify-center gap-4"><Link to="/" className="rounded-lg bg-blue-700 px-5 py-3 font-semibold text-white">Ir al inicio</Link><Link to={`/pedido/${result.tracking_code}`} className="rounded-lg border px-5 py-3 font-semibold">Ver compra</Link></div>
  </div></div>
}
