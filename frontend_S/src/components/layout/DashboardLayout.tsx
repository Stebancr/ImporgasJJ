...
const navigation: NavItem[] = [
  ...
  { name: 'CRM Chat',    href: '/dashboard/chat',       icon: MessageSquare, roles: ['admin'] },
  {
    ...
    roles: ['admin'],
    children: [
      { name: 'Cotizaciones',       href: '/dashboard/gestion/cotizaciones', icon: ClipboardList },
      { name: 'Facturas',           href: '/dashboard/gestion/facturas',     icon: FileText },
    ],
  },
]
...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    const fetchCount = () =>
      import('@/services/chat').then(({ adminChatService }) =>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center gap-2 px-4 py-5 border-b">
            <Link to="/" aria-label="ImporGas JJ, inicio"><img src="/logo_imporgas.svg" alt="ImporGas JJ" className="w-40 h-10 object-contain" /></Link>
            <button
              onClick={() => setSidebarOpen(false)}
              aria-label="Cerrar navegación"
              className="ml-auto h-11 w-11 shrink-0 flex items-center justify-center lg:hidden text-muted-foreground hover:text-foreground"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
